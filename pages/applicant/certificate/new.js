...
class UploadCertificate extends Component {
  ...
  // }
    ...
    try {
      ...
      await factory.methods
        .addCertificate(name,description,result[0].hash)
        .send({ from: accounts[0] });
      Router.pushRoute(`/applicant/${this.props.address}/certificate`);
    } catch (err) {
  ...
  render() {
    return (
      <Layout>
        <Link route={`/applicant/${this.props.address}/certificate`}>
          <a>Back</a>
        </Link>
        <h3>Upload a Certificate for verification</h3>
        <Form onSubmit={this.onSubmit} error={!!this.state.errorMessage}>
          ...
          <Form.Field>
            <label>Description of the Certificate</label>
            <Input
