...
class RequestRowCVerifier extends Component {
    state = {
        loadingv: false,
        loadingr: false,
        errorMessagev: "",
  ...
  onVerify = async () => {
    ...
    try{
    await factory.methods.verifyCertificate(this.props.hash, 1).send({
      from: accounts[0],
    });
    Router.pushRoute(`/certverifier/${this.props.add}/verify`);
}catch(err){
  ...
  onReject = async () => {
    ...
    await factory.methods.verifyCertificate(this.props.hash, 2).send({
      from: accounts[0],
    });
    Router.pushRoute(`/certverifier/${this.props.add}/verify`);
}catch(err){
...
export default RequestRowCVerifier;
